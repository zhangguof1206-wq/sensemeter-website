...
export function AccessoryCategoryCard({ category, locale }: { category: AccessoryCategory; locale: Locale }) {
  ...
  return (
    <Link className="card group flex h-full flex-col overflow-hidden bg-white transition hover:border-accent/50 hover:shadow-lg" href={localizedPath(locale, `/accessories/${category.slug}`)}>
      <div className="grid aspect-[4/3] place-items-center border-b border-line bg-white p-6">
        <img className="accessory-category-image max-h-[82%] max-w-[82%] object-contain transition duration-300 group-hover:scale-[1.02]" src={category.image} alt={category.title[locale]} />
      </div>
  ...
  return (
    <Link className="card group flex h-full flex-col overflow-hidden transition hover:border-accent/50 hover:shadow-lg" href={localizedPath(locale, path)}>
      <div className="grid aspect-[4/3] place-items-center overflow-hidden border-b border-line bg-[#eef2f5] p-6">
        <img className="accessory-product-image max-h-[82%] max-w-[82%] object-contain transition duration-300 group-hover:scale-[1.02]" src={product.image} alt={product.title[locale]} />
      </div>
