...
const urls = output
  .split(/\r?\n/)
  .map((line) => line.trim())
  .filter((line) => line.startsWith("https://sensemeter.ru"));

const expectedCount = 14 + productSlugs.length * 2;
const requiredUrls = [
  "https://sensemeter.ru/",
...
const checks = [
  {
    name: "Yandex URL list has no duplicate URLs",
    pass: new Set(urls).size === urls.length
  },
  {
    name: `Yandex URL list includes ${expectedCount} sitemap URLs`,
    pass: urls.length === expectedCount
  },
  {
    name: "Yandex URL list includes every core and product URL",
    pass: requiredUrls.every((url) => urls.includes(url))
  },
  {
    name: "Yandex URL list labels manual submission order",
    pass: output.includes("1. RU priority pages") &&
      output.includes("2. RU product pages") &&
      output.includes("3. EN priority pages") &&
      output.includes("4. EN product pages") &&
      output.includes("5. Optional legal pages")
  },
  {
    name: "Yandex URL list excludes noindex thank-you pages",
    pass: !output.includes("/thank-you")
  }
];
